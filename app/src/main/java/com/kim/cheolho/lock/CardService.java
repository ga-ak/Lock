...
public class CardService extends HostApduService {
    ...
    private static final byte[] WATING_SERVER = HexStringToByteArray("00FF");
    private static final byte[] NFC_ADDRESS = HexStringToByteArray("FF00");
    public String receivedKey = "FFFF";
    ...
    public static byte[] BuildSelectApdu(String aid) {
        ...
        String temp = ByteArrayToHexString(HexStringToByteArray(SELECT_APDU_HEADER + String.format("%02X",
                aid.length() / 2) + aid));
    ...
    public static byte[] cutByteTail(byte[] inputBytes) {
        byte[] result = new byte[2];
        for (int i = 0; i < result.length; i++) {
            result[i] = inputBytes[inputBytes.length-2+i];
        }
        return result;
    }

    public static byte[] cutByteNfc(byte[] inputBytes) {
    ...
    public class MyAsyncTask extends AsyncTask<Void, Void, String> {
        ...
        public MyAsyncTask(String url, String data) {
            this.url = url;
            this.data = data;
        }
